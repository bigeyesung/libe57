      ...
      switch ( imageType )
      {
         ...
         case E57_JPEG_IMAGE:
         {
            if ( image.isDefined( "jpegImage" ) )
            {
               ...
               std::string fileprefix = "/home/chenhsi/Projects/mine/libE57Format/test/test_e57_";
               std::string s = std::to_string(countInd);
               std::string postfix = ".jpg";
               std::string filepath = fileprefix + s + postfix;
               int n = filepath.length();
               //char char_array[n + 1];
               //strcpy(char_array, filepath.c_str());  
               FILE* fp = fopen(filepath.c_str(),"wb");
               fwrite(pBuffer,(size_t)count,1,fp);
               fclose(fp);
               countInd += 1;
               transferred = count;
            }
